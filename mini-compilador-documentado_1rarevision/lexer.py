...
# Lista que acumula errores léxicos durante cada llamada a tokenize()
# Formato de cada entrada: (lineno, caracter_ilegal)
_lex_errors = []
...
def t_error(t):
    ...
    """
    _lex_errors.append((t.lineno, t.lexpos, t.value[0]))
...
def tokenize(source_code: str, verbose: bool = True):
    ...
    Retorna:
        token_list   (list[LexToken])       : lista de tokens en orden de aparición
        errores      (list[tuple[int,str]]) : lista de (lineno, caracter_ilegal)
    """
    global _lex_errors
    _lex_errors = []   # Reinicia el colector antes de cada tokenización
    ...
    return token_list, list(_lex_errors)