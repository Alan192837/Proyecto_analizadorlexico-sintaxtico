...
Propósito general:
    Orquesta el pipeline completo de compilación e interpretación:
        ...
        3. Análisis semántico (semantic.py) → verifica tipos, ámbitos y argumentos
        4. Ejecución     (interpreter) → recorre el AST y produce resultados
        5. Reportes      (este módulo) → genera archivos de salida

    REGLA IMPORTANTE DEL PIPELINE: un programa con errores semánticos NO se
    ejecuta. Si la fase 3 encuentra aunque sea un error, la fase 4 se cancela;
    ejecutar código con tipos incorrectos solo produciría fallos confusos en
    tiempo de ejecución en lugar del diagnóstico preciso que ya se tiene.
...
Uso desde la terminal (dentro de la carpeta del proyecto):
    ...
    python main.py --no-sem              → omite el análisis semántico
...
Archivos generados automáticamente en cada ejecución:
    ...
    progfte.tab           → tabla de tokens en formato de texto (incluye errores)
    progfte.sem           → reporte del análisis semántico (errores o "sin errores")
    progfte.sim           → tabla de símbolos con tipo, ámbito, línea y columna
=============================================================================
"""
...
# Importa las cuatro fases del pipeline
from lexer       import tokenize          # Fase 1: análisis léxico
from parser      import parse             # Fase 2: análisis sintáctico
from semantic    import SemanticAnalyzer  # Fase 3: análisis semántico
from interpreter import Interpreter       # Fase 4: ejecución
...
def generar_codigo_depurado(tokens, filename="progfte.dep"):
    ...
    try:
        ...
        print(f"[Sistema] Codigo fuente depurado generado en: {filename}")

    except Exception as e:
        print(f"[Error] No se pudo generar el codigo depurado: {e}")
...
def run_source(source: str, show_tokens: bool = False, show_ast: bool = False,
               show_symbols: bool = True, run_semantic: bool = True, label: str = ""):
    """
    ...
    Parámetros:
        ...
        show_symbols (bool): si True, muestra la tabla de símbolos al terminar
        run_semantic (bool): si False, omite el análisis semántico (flag --no-sem)
        label        (str) : texto descriptivo del programa

    Retorna:
        bool: True si el programa llegó a ejecutarse sin errores de ninguna fase
    """
    sep = "=" * 60
    print(f"\n{sep}")
    if label:
    ...
    # Solo tiene sentido si el parser logró construir un AST: sin árbol no hay
    # nada que verificar, y el error ya fue reportado por la fase anterior.
    errores_sem = []
    analizador  = None

    if ast is not None and run_semantic:
        print("\n-- ANALISIS SEMANTICO --")
        analizador  = SemanticAnalyzer()
        errores_sem = analizador.analyze(ast)

    # ── Fase 4: Ejecución ─────────────────────────────────────────────────
    # Un programa con errores semánticos NO se ejecuta: ya se sabe que sus
    # tipos son incorrectos, así que ejecutarlo solo daría fallos confusos.
    ejecutado = False
    interp    = None

    if ast is None:
        print("\n[Sistema] No se ejecuta: hubo errores lexicos o sintacticos.")
    elif errores_sem:
        print("\n[Sistema] Ejecucion cancelada: corrige los "
              f"{len(errores_sem)} error(es) semantico(s) listados arriba.")
    else:
        print("\n-- SALIDA --")
        interp = Interpreter()
        try:
    ...
    # Reporte de errores de la fase semántica
    if analizador is not None:
        analizador.save_report("progfte.sem")

    # Tabla de símbolos. Se prefiere la del intérprete cuando el programa llegó
    # a ejecutarse, porque contiene lo mismo que la del analizador (nombre,
    # tipo, ámbito y posición) MÁS el valor final de cada variable.
    # Si el programa no se ejecutó, se guarda la del analizador, que es la
    # única disponible y sigue documentando tipos y ámbitos.
    if interp is not None:
        interp.save_symbols("progfte.sim")
    elif analizador is not None:
        analizador.tabla.save_to_file("progfte.sim")
...
def main():
    ...
    hide_semantic = '--no-sem' in sys.argv
    ...
    if source_file:
        ...
        run_source(source,
                   ...
                   run_semantic=not hide_semantic,
                   label=f"Archivo: {source_file}")
        return

    print("\n" + "#" * 60)
    print("  MINI-COMPILADOR - Calculadora Extendida con PLY")
