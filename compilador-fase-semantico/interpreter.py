...
#
# ÁMBITOS:
#   Los cuerpos de 'si', 'sino' y 'mientras' abren un ámbito propio mediante
#   enter_scope()/exit_scope() de la tabla de símbolos, exactamente igual que
#   hace el analizador semántico. Que ambas fases apliquen la misma regla de
#   alcance es lo que garantiza que un programa aceptado por el analizador se
#   ejecute sin sorpresas.
#
...
class Interpreter:
    ...
    # Tope de vueltas permitidas en un bucle 'mientras'. Un programa cuya
    # condición nunca se vuelve falsa colgaría el intérprete indefinidamente;
    # con este límite se corta con un mensaje claro en lugar de quedar mudo.
    #
    # 100 000 es holgadamente más de lo que necesita cualquier programa de este
    # lenguaje (que no tiene arreglos ni funciones), y a la vez lo bastante bajo
    # para que un bucle sin fin se detecte en un par de segundos en lugar de
    # llenar la consola durante minutos.
    ...
    def _execute(self, node: ASTNode):
        ...
        Parámetros:
            node (ASTNode): nodo de sentencia del AST. Se espera que sea una instancia
                            de VarDecl, Assign, PrintStmt, ReadStmt, IfStmt o WhileStmt.
    ...
    def _exec_decl(self, node: VarDecl):
        ...
        # conservando su posición en el fuente para el reporte final
    ...
    def _exec_mostrar(self, node: PrintStmt):
        """
        Ejecuta la instrucción de salida del lenguaje (ej: mostrar(x + 1)).

        Evalúa la expresión y adapta la representación de los booleanos a la
        sintaxis del mini-lenguaje (verdadero/falso) en lugar de la de Python
        (True/False).
        ...
        """
        print(self._a_texto(self._eval(node.expr)))

    @staticmethod
    def _a_texto(valor) -> str:
        """
        Convierte un valor a la representación textual del mini-lenguaje.

        La única diferencia con str() de Python está en los booleanos: Python
        los escribe 'True'/'False' y este lenguaje 'verdadero'/'falso'. Tener
        la conversión en un solo sitio garantiza que la salida sea coherente
        tanto al imprimir un valor suelto como al concatenarlo dentro de una
        cadena más larga.

        Parámetros:
            valor (any): valor Python resultante de evaluar una expresión

        Retorna:
            str: el texto que se mostrará al usuario
        """
    ...
    def _exec_si(self, node: IfStmt):
        ...
        if condicion:
            ...
            # dentro de su propio ámbito para que las variables declaradas aquí
            # no sobrevivan al bloque (misma regla que aplicó el semántico).
            self.symbols.enter_scope()
            try:
                for stmt in node.then_body:
                    self._execute(stmt)
            finally:
                # try/finally garantiza que el ámbito se cierra aunque una
                # sentencia lance RuntimeError; sin esto, un error dejaría la
                # pila de ámbitos desbalanceada.
    ...
    def _exec_mientras(self, node: WhileStmt):
        """
        Ejecuta un bucle (ej: mientras (i < 10) { ... }).

        La condición se reevalúa ANTES de cada vuelta, de modo que si es falsa
        desde el principio el cuerpo no se ejecuta ninguna vez.

        Cada vuelta abre y cierra su propio ámbito: una variable declarada
        dentro del cuerpo nace y muere en cada iteración, que es justo lo que
        el analizador semántico verificó.

        Parámetros:
            node (WhileStmt): nodo AST del bucle, con campos:
                - condition (ASTNode)  : condición de permanencia.
                - body (list[ASTNode]) : sentencias que se repiten.

        Retorna:
            None. El efecto son las sentencias ejecutadas en cada vuelta.

        Lanza:
            RuntimeError si se supera MAX_ITERACIONES, señal casi segura de un
            bucle infinito (por ejemplo, olvidar incrementar el contador).
        """
        vueltas = 0

        while self._eval(node.condition):
            vueltas += 1
            if vueltas > self.MAX_ITERACIONES:
                raise RuntimeError(
                    f"[Interprete] El bucle 'mientras' de la linea {node.lineno} "
                    f"supero {self.MAX_ITERACIONES} iteraciones. "
                    f"Es probable que la condicion nunca se vuelva falsa: "
                    f"revisa que alguna variable de la condicion cambie dentro del cuerpo."
                )
    ...
    def _eval_binop(self, node: BinOp):
        ...
        if op == '+':
            ...
            #
            # Se usa _a_texto y no str() para que un valor lógico se concatene
            # como 'verdadero'/'falso' y no como el 'True'/'False' de Python:
            # el programa está escrito en este lenguaje y su salida también
            # debe estarlo.
            if isinstance(izq, str) or isinstance(der, str):
                return self._a_texto(izq) + self._a_texto(der)
            return izq + der
    ...
    def _verificar_tipo(self, tipo_declarado: str, valor, nombre_var: str):
        ...
        try:
            ...
            if tipo_declarado == 'cadena':
                # _a_texto nunca lanza excepción y, a diferencia de str(),
                # escribe los booleanos con las palabras del lenguaje. Este
                # caso solo se alcanza con --no-sem, porque el analizador
                # semántico rechaza antes 'cadena c = verdadero;'.
