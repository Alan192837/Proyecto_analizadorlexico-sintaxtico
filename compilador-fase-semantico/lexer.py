...
PALABRAS RESERVADAS DEL LENGUAJE (NO usar como nombres de variable):
    ...
    mientras      → bucle                (equivale a while)
...
reserved = {
    ...
    'mientras'      : 'MIENTRAS',     # Inicio de bucle (equivale a while)
...
def t_NUMERO_DECIMAL(t):
    r'\d+\.\d+'
    # Captura numeros decimales para dar un error claro en vez de tokenizar
    # el punto como caracter ilegal. Debe definirse antes de t_NUMERO para
    # que PLY no consuma solo la parte entera primero.
    print(f"[Lexico] Numero decimal no soportado en linea {t.lineno}: "
          f"'{t.value}'. Este lenguaje solo admite enteros.")
    t.type  = 'NUMERO'
...
# =============================================================================
# 6. CÁLCULO DE COLUMNA
#    PLY lleva la cuenta de la LÍNEA (t.lineno) automáticamente gracias a
#    t_newline, pero NO calcula la columna: solo guarda 'lexpos', que es el
#    desplazamiento absoluto del token en caracteres desde el inicio del fuente.
#
#    La columna se deduce restando a lexpos la posición del último salto de
#    línea anterior al token.
# =============================================================================

def find_column(source_code: str, lexpos: int) -> int:
    """
    Convierte una posición absoluta (lexpos) en número de columna.

    El analizador semántico necesita la columna para poder reportar errores
    con la precisión que exige la práctica ("errores claros: línea, columna").

    Funcionamiento:
        rfind('\\n', 0, lexpos) devuelve el índice del último salto de línea
        ANTES del token. Restándolo a lexpos se obtiene cuántos caracteres hay
        desde el inicio de la línea hasta el token.

        Si el token está en la primera línea no hay ningún '\\n' previo y
        rfind devuelve -1; la resta lexpos-(-1) = lexpos+1 da exactamente la
        columna 1-based correcta, así que no hace falta un caso especial.

    Parámetros:
        source_code (str): texto completo del programa analizado
        lexpos      (int): desplazamiento absoluto del token (atributo de LexToken)

    Retorna:
        int: número de columna empezando en 1 (como lo muestran los editores)
    """
    inicio_linea = source_code.rfind('\n', 0, lexpos)
    return lexpos - inicio_linea


# =============================================================================
# 7. FUNCIÓN DE UTILIDAD — tokenize()
# =============================================================================
